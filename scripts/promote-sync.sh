...
# Shell lint on the exact files the (local) stack touches (C5/F8, M10 2026-08-22).
# The script's default worktree-vs-HEAD path inspects nothing on a committed
# candidate; the range derives from the tag so future syncs lint the right span.
lint_base="refs/upstream-tags/$TAG"
git rev-parse --verify --quiet "$lint_base" >/dev/null || lint_base="$TAG"
stack_shell_files=$(git diff --name-only --diff-filter=ACMR "$lint_base" HEAD -- '*.sh')
if [[ -z "$stack_shell_files" ]]; then
  echo "error: stack-touched shell list is empty — lint range derivation is wrong (C8: empty inspection is not a pass)" >&2
  exit 1
fi
# newline-split file list; repo paths carry no spaces
# shellcheck disable=SC2086
scripts/lint-shell.sh $stack_shell_files >/dev/null \
  || { echo "error: shell lint failed on stack-touched files" >&2; exit 1; }
echo "  shell lint (stack-touched): PASS"
...
for t in tests/hooks/test-session-start.sh tests/shell-lint/test-lint-shell.sh \
  ...
  # C7 (M10 2026-08-22): a missing declared test fails the gate — with all four
  # absent this loop once printed PASS having run nothing
  [[ -f "$t" ]] || { echo "error: declared deterministic test missing on candidate: $t" >&2; exit 1; }
...
# A2 scoped bypass (M10 2026-08-22): this one push moves main to a candidate whose
# diff carries upstream's new Python; the machine-global pre-push lint would block
# it. Bypass THIS push only — the publication-commit push below stays hook-checked.
git -c core.hooksPath=/dev/null push --force-with-lease="refs/heads/main:$main_before" origin "$candidate_sha:refs/heads/main"
...
# C9 (M10 2026-08-22): `git add -A` below must stage ONLY bump-version's writes —
# refuse untracked/modified litter rather than ship it inside the publication commit
if [[ -n "$(git status --porcelain)" ]]; then
  echo "error: worktree not clean before release cut — refusing to stage stray files into the publication commit:" >&2
